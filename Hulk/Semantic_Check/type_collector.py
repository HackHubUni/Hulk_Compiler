...
class TypeCollector:
    ...
    @visitor.on("node")
    def visit(self, node: AstNode, scope: HulkScopeLinkedNode, from_where: str = ""):
        pass

    @visitor.when(ProgramNode)
    def visit(
        self, node: ProgramNode, scope: HulkScopeLinkedNode, from_where: str = ""
    ):
        for declaration in node.declarations:
            self.visit(declaration, self.global_scope, "In the program")

    @visitor.when(TypeDeclarationNode)
    def visit(
        self, node: TypeDeclarationNode, scope: HulkScopeLinkedNode, from_where: str
    ):
        from_where += f" in the declaration of the type with name '{node.type_name}',"
        if node.type_name in builtin_types:
            self.errors.append(
                SemanticError(
                    f"{from_where} the Type '{node.type_name}' is a builtin type and cannot be redefined"
                )
            )
            node.type_name = get_unique_name_with_guid(node.type_name)
        elif scope.is_type_or_protocol_defined(node.type_name):
            self.errors.append(
                SemanticError(
                    f"{from_where} the name '{node.type_name}' is already defined in another Type or Protocol"
                )
            )
            node.type_name = get_unique_name_with_guid(node.type_name)
    ...
    def visit(
        self, node: ProtocolDeclarationNode, scope: HulkScopeLinkedNode, from_where: str
    ):
        from_where += (
            f" in the declaration of the protocol with name '{node.protocol_name}',"
        )
        if node.protocol_name in builtin_protocols:
            self.errors.append(
                SemanticError(
                    f"{from_where} the Protocol '{node.protocol_name}' is a builtin protocol and cannot be redefined"
                )
            )
            node.protocol_name = get_unique_name_with_guid(node.protocol_name)
        elif scope.is_type_or_protocol_defined(node.protocol_name):
            self.errors.append(
                SemanticError(
                    f"{from_where} the name '{node.protocol_name}' is already defined in another type or protocol"
                )
            )
            node.protocol_name = get_unique_name_with_guid(node.protocol_name)
    ...
    def visit(
        self, node: FunctionDeclarationNode, scope: HulkScopeLinkedNode, from_where: str
    ):
        from_where += (
            f" in the declaration of the function with name '{node.function_name}'"
        )
        if node.function_name in builtin_functions:
            self.errors.append(
                SemanticError(
                    f"{from_where} the name '{node.function_name} 'is a builtin function and cannot be redefined"
                )
            )
            ...
            self.errors.append(
                SemanticError(
                    f"{from_where} the name '{node.function_name}' is already defined"
                )
            )
            node.function_name = get_unique_name_with_guid(node.function_name)
